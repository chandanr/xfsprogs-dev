	...
	struct xfs_mount	*mp)
{
	...
	/*
	 * printk("%s:%d: da_blks = %d; bmbt_blks = %d; da_blks res = %d; bmbt_blks res = %d; total = %d.\n",
	 * 	__func__, __LINE__,
	 * 	da_blks, bmbt_blks, xfs_calc_buf_res(da_blks, XFS_FSB_TO_B(mp, 1)),
	 * 	xfs_calc_buf_res(bmbt_blks, XFS_FSB_TO_B(mp, 1)), total);
	 */
