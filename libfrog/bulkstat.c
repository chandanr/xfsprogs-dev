...
{
	struct xfs_bulkstat_req_v6	*req;
	int				ret;
	...
	memcpy(bulkstat, req->bulkstat, sizeof(struct xfs_bulkstat_v6));
free:
	free(req);
	return ret;
}
...
xfrog_bulkstat_single5(
	...
	struct xfs_bulkstat_v6		*bulkstat)
{
	struct xfs_bulkstat_req_v5	*req5;
	int				ret;

	if (flags & ~(XFS_BULK_IREQ_SPECIAL))
		return -EINVAL;

	req5 = calloc(1, XFS_BULKSTAT_V5_REQ_SIZE(1));
	if (!req5)
		return -ENOMEM;

        req5->hdr.icount = 1;
	req5->hdr.ino = ino;
	req5->hdr.flags = flags;

        ret = ioctl(xfd->fd, XFS_IOC_BULKSTAT_V5, req5);
	if (ret) {
		ret = -errno;
		goto free;
	}

	if (req5->hdr.ocount == 0) {
		ret = -ENOENT;
		goto free;
	}

	xfrog_bulkstat_v5_to_v6(xfd, bulkstat, req5->bulkstat);
...
xfrog_bulkstat5(
	struct xfs_fd			*xfd,
	struct xfs_bulkstat_req_v6	*req6)
{
	struct xfs_bulkstat_req_v5	*req5;
	struct xfs_bulk_ireq		*hdr;
	unsigned int			i;
	...
	req5 = calloc(1, XFS_BULKSTAT_V5_REQ_SIZE(hdr->icount));
	if (!req5)
		return -ENOMEM;

        memcpy(&req5->hdr, hdr, sizeof(*hdr));
	...
	for (i = 0; i < req5->hdr.ocount; i++) {
		struct xfs_bulkstat_v6 *bs6;
		struct xfs_bulkstat_v5 *bs5;

		bs6 = &req6->bulkstat[i];
		bs5 = &req5->bulkstat[i];
		xfrog_bulkstat_v5_to_v6(xfd, bs6, bs5);
	}

	memcpy(hdr, &req5->hdr, sizeof(*hdr));
...
out_teardown:
	return xfrog_bulk_req_v1_cleanup(xfd, &req->hdr, &bulkreq,
			...
			&req->bulkstat, sizeof(struct xfs_bulkstat_v6),
...
/* Convert bulkstat data from v6 format to v1 format. */
int
xfrog_bulkstat_v6_to_v1(
	struct xfs_fd			*xfd,
	struct xfs_bstat		*bs1,
	const struct xfs_bulkstat_v6	*bs6)
{
	if (bs6->bs_aextents > UINT16_MAX ||
	    bs6->bs_extents > INT32_MAX ||
	    cvt_off_fsb_to_b(xfd, bs6->bs_extsize_blks) > UINT32_MAX ||
	    cvt_off_fsb_to_b(xfd, bs6->bs_cowextsize_blks) > UINT32_MAX ||
	    time_too_big(bs6->bs_atime) ||
	    time_too_big(bs6->bs_ctime) ||
	    time_too_big(bs6->bs_mtime))
		return -ERANGE;

	bs1->bs_ino = bs6->bs_ino;
	bs1->bs_mode = bs6->bs_mode;
	bs1->bs_nlink = bs6->bs_nlink;
	bs1->bs_uid = bs6->bs_uid;
	bs1->bs_gid = bs6->bs_gid;
	bs1->bs_rdev = bs6->bs_rdev;
	bs1->bs_blksize = bs6->bs_blksize;
	bs1->bs_size = bs6->bs_size;
	bs1->bs_atime.tv_sec = bs6->bs_atime;
	bs1->bs_mtime.tv_sec = bs6->bs_mtime;
	bs1->bs_ctime.tv_sec = bs6->bs_ctime;
	bs1->bs_atime.tv_nsec = bs6->bs_atime_nsec;
	bs1->bs_mtime.tv_nsec = bs6->bs_mtime_nsec;
	bs1->bs_ctime.tv_nsec = bs6->bs_ctime_nsec;
	bs1->bs_blocks = bs6->bs_blocks;
	bs1->bs_xflags = bs6->bs_xflags;
	bs1->bs_extsize = cvt_off_fsb_to_b(xfd, bs6->bs_extsize_blks);
	bs1->bs_extents = bs6->bs_extents;
	bs1->bs_gen = bs6->bs_gen;
	bs1->bs_projid_lo = bs6->bs_projectid & 0xFFFF;
	bs1->bs_forkoff = bs6->bs_forkoff;
	bs1->bs_projid_hi = bs6->bs_projectid >> 16;
	bs1->bs_sick = bs6->bs_sick;
	bs1->bs_checked = bs6->bs_checked;
	bs1->bs_cowextsize = cvt_off_fsb_to_b(xfd, bs6->bs_cowextsize_blks);
	bs1->bs_dmevmask = 0;
	bs1->bs_dmstate = 0;
	bs1->bs_aextents = bs6->bs_aextents;
	return 0;
}

/* Convert bulkstat data from v5 format to v1 format. */
int
xfrog_bulkstat_v5_to_v1(	/* chandan: Is this required? */
	struct xfs_fd			*xfd,
	struct xfs_bstat		*bs1,
	...
	const struct xfs_bulkstat_v5	*bs5)
{
	...
	bs6->bs_version = XFS_BULKSTAT_VERSION_V5;

	bs6->bs_ino = bs5->bs_ino;
	bs6->bs_mode = bs5->bs_mode;
	bs6->bs_nlink = bs5->bs_nlink;
	bs6->bs_uid = bs5->bs_uid;
	bs6->bs_gid = bs5->bs_gid;
	bs6->bs_rdev = bs5->bs_rdev;
	bs6->bs_blksize = bs5->bs_blksize;
	bs6->bs_size = bs5->bs_size;
	bs6->bs_atime = bs5->bs_atime;
	bs6->bs_mtime = bs5->bs_mtime;
	bs6->bs_ctime = bs5->bs_ctime;
	bs6->bs_atime_nsec = bs5->bs_atime_nsec;
	bs6->bs_mtime_nsec = bs5->bs_mtime_nsec;
	bs6->bs_ctime_nsec = bs5->bs_ctime_nsec;
	bs6->bs_blocks = bs5->bs_blocks;
	bs6->bs_xflags = bs5->bs_xflags;
	bs6->bs_extsize_blks = bs5->bs_extsize_blks;
	bs6->bs_extents = bs5->bs_extents;
	bs6->bs_gen = bs5->bs_gen;
	bs6->bs_projectid = bs5->bs_projectid;
	bs6->bs_forkoff = bs5->bs_forkoff;
	bs6->bs_sick = bs5->bs_sick;
	bs6->bs_checked = bs5->bs_checked;
	bs6->bs_cowextsize_blks = bs5->bs_cowextsize_blks;
	bs6->bs_aextents = bs5->bs_aextents;
}

/* Convert bulkstat data from v1 format to v6 format. */
void
...
/* Allocate a bulkstat v6 request.  Returns zero or a negative error code. */
int
xfrog_bulkstat_alloc_req(
	uint32_t			nr,
	uint64_t			startino,
	struct xfs_bulkstat_req_v6 	**preq)
{
	struct xfs_bulkstat_req_v6	*breq;

	breq = calloc(1, XFS_BULKSTAT_V6_REQ_SIZE(nr));
	if (!breq)
		return -ENOMEM;
...
/* Allocate a bulkstat v5 request.  Returns zero or a negative error code. */
int
xfrog_bulkstat_v5_alloc_req(	/* chandan: is this required? */
	uint32_t			nr,
	uint64_t			startino,
