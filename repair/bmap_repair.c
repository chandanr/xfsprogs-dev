	...
	void			*priv)
{
	...
	/*
	 * Count the old bmbt blocks we find so we can reduce the inode block
	 * count later.  The old bmbt blocks themselves will show up in the
	 * free map, so we don't need to delete them.
	 */
	...
	if (!xfs_verify_fileext(mp, rec->rm_offset, rec->rm_blockcount))
		return -EFSCORRUPTED;

	if (!xfs_verify_fsbext(mp, fsbno, rec->rm_blockcount))
		return -EFSCORRUPTED;
	...
	int			*dirty)
{
	 ...
	 */
	error = -libxfs_imap_to_bp(mp, sc.tp, &sc.ip->i_imap, dinop, ino_bpp, 0);
	if (error)
		goto out_trans;
