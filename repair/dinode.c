...
_("would have cleared inode %" PRIu64 " attributes\n"), ino_num);
	...
	if (XFS_DFORK_NEXTENTS(dino, XFS_ATTR_FORK, mp->m_32bit_naextents) != 0)  {
		if (no_modify)
			return(1);
	...
	int			check_dups)
{
	...
	*tot = 0;
	numrecs = XFS_DFORK_NEXTENTS(dip, whichfork, mp->m_32bit_naextents);
...
_("too many attr fork extents (%" PRIu64 ") in inode %" PRIu64 "\n"),
	...
	if (anextents != XFS_DFORK_NEXTENTS(dino, XFS_ATTR_FORK, mp->m_32bit_naextents))  {
		if (!no_modify)  {
			do_warn(
_("correcting anextents for inode %" PRIu64 ", was %d - counted %" PRIu64 "\n"),
				lino,
XFS_DFORK_NEXTENTS(dino, XFS_ATTR_FORK, mp->m_32bit_naextents), anextents);
...
XFS_DFORK_NEXTENTS(dino, XFS_ATTR_FORK, mp->m_32bit_naextents),
				lino, anextents);
		}
	}
	...
	int		*retval)
{
	...
	*anextents = XFS_DFORK_NEXTENTS(dino, XFS_ATTR_FORK, mp->m_32bit_naextents);
