...
_("too many attr fork extents (%" PRIu64 ") in inode %" PRIu64 "\n"),
	...
	if (anextents != XFS_DFORK_NEXTENTS(&mp->m_sb, dino, XFS_ATTR_FORK))  {
		if (!no_modify)  {
			do_warn(
_("correcting anextents for inode %" PRIu64 ", was %d - counted %" PRIu64 "\n"),
				lino,
XFS_DFORK_NEXTENTS(&mp->m_sb, dino, XFS_ATTR_FORK), anextents);
...
XFS_DFORK_NEXTENTS(&mp->m_sb, dino, XFS_ATTR_FORK),
				lino, anextents);
		}
	}
