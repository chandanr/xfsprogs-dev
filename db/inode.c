...
static int	inode_core_big_nextents_count(void *obj, int startoff);
...
static int	inode_core_big_anextents_count(void *obj, int startoff);
...
const field_t	inode_core_flds[] = {
	...
	{ "projid_hi", FLDT_UINT16D, OI(COFF(projid_hi)),
	  inode_core_projid_count, FLD_COUNT, TYP_NONE },
	{ "big_nextents", FLDT_UINT64D, OI(COFF(big_nextents)),
	  inode_core_big_nextents_count, FLD_COUNT, TYP_NONE },
	...
	{ "big_naextents", FLDT_UINT32D, OI(COFF(big_anextents)),
	  inode_core_big_anextents_count, FLD_COUNT, TYP_NONE },
...
static int
inode_core_big_anextents_count(
	void			*obj,
	int			startoff)
{
	struct xfs_dinode	*dic;
