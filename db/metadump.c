	...
	int	len)
{
	...
	block_index = metadump.mv1.block_index;
	block_buffer = metadump.mv1.block_buffer;
	cur_index = metadump.mv1.cur_index;
	num_indices = metadump.mv1.num_indices;
	...
	struct xfs_dinode	*dip)
{
		...
		if (namelen == 0) {
			...
			if (metadump.show_warnings)
				print_warning("entry length in dir inode %llu "
					"overflows space", (long long)metadump.cur_ino);
			if (i != sfp->count - 1)
				break;
			namelen = ino_dir_size - ((char *)&sfep->name[0] -
					 (char *)sfp);
	...
	if (metadump.zero_stale_data && (ino_dir_size < XFS_DFORK_DSIZE(dip, mp)))
	...
	struct xfs_dinode		*dip)
{
	...
	if (metadump.zero_stale_data && (ino_attr_size < XFS_DFORK_ASIZE(dip, mp)))
		...
		if (i > 0 && op + cp > o) {
			if (metadump.show_warnings)
				print_warning("bmap extent %d in %s ino %llu "
					...
					"ended at %llu", i,
					typtab[btype].name, (long long)metadump.cur_ino,
					o, op + cp - 1);
			break;
		}
		...
		if (bno == 0 || bno > mp->m_sb.sb_agblocks ||
			...
			if (metadump.show_warnings)
				print_warning("invalid block number (%u/%u) "
						...
						bno, (long long)metadump.cur_ino,
						typtab[btype].name);
			continue;
		}
	...
	typnm_t			itype)
{
	...
	if (metadump.zero_stale_data && (used < XFS_DFORK_SIZE(dip, mp, whichfork)))
	...
	char 		**argv)
{
	...
	while ((c = getopt(argc, argv, "aegm:ov:w")) != EOF) {
		switch (c) {
			...
			case 'g':
				...
				break;
			case 'm':
				metadump.max_extent_size = (int)strtol(optarg, &p, 0);
				if (*p != '\0' || metadump.max_extent_size <= 0) {
	...
	/* copy log */
	if (!exitcode && (mp->m_sb.sb_logstart || metadump.version == 2))
